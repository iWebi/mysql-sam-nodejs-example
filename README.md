...
```bash
mysql-sam-nodejs-example$ sam build
```
...
```bash
mysql-sam-nodejs-example$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mysql-sam-nodejs-example$ sam local start-api
mysql-sam-nodejs-example$ curl http://localhost:3000/
```
...
```bash
mysql-sam-nodejs-example$ sam logs -n HelloWorldFunction --stack-name mysql-sam-nodejs-example --tail
```
...
```bash
mysql-sam-nodejs-example$ cd hello-world
...
```bash
sam delete --stack-name mysql-sam-nodejs-example
```
